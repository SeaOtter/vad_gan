...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    # ]
    # input_height = generator_inputs.get_shape()[1].value
    # layer_specs_full = [
    #         a.ngf * 2, # encoder_2: [batch, 128, 128, ngf] => [batch, 64, 64, ngf * 2]
    #         a.ngf * 4, # encoder_3: [batch, 64, 64, ngf * 2] => [batch, 32, 32, ngf * 4]
    #         a.ngf * 8, # encoder_4: [batch, 32, 32, ngf * 4] => [batch, 16, 16, ngf * 8]
    ...
    #         a.ngf * 8, # encoder_8: [batch, 2, 2, ngf * 8] => [batch, 1, 1, ngf * 8]
    #     ]
    # n = int(np.log2(input_height))-1
    # print('n = %d' % n)
    # layer_specs = layer_specs_full[:n]
    layer_specs = a.gen_layers_specs1
    ...
    #
    # layer_specs = layer_specs_full[-n:]
    layer_specs = a.gen_layers_specs2
...
def create_model(inputs, targets):
    ...
    def create_discriminator(discrim_inputs, discrim_targets):
        # n_layers = 3
        layers = []
        # n = discrim_inputs.get_shape()[1].value
        ...
        with tf.variable_scope("layer_1"):
            ...
            convolved = discrim_conv(input, a.ndf, stride=a.dis_layers_specs[0][2])
        ...
        # else:
        #     n_layers = -1
        # print('n = %d n_layers = %d' % (n, n_layers))
        #
        # if n_layers >0:
        ...
        #             layers.append(rectified)
        dis_layers_specs = a.dis_layers_specs[1:]
        if len(dis_layers_specs)>0:

            for i in range(len(dis_layers_specs)):
                    with tf.variable_scope("layer_%d" % (len(layers) + 1)):
                        out_channels = dis_layers_specs[i][1]
                        # out_channels = a.ndf * min(2**(i+1), 8)
                        # stride = 1 if i == len(dis_layers_specs) - 1 else 2  # last layer here has stride 1
                        stride = dis_layers_specs[i][2]
                        convolved = discrim_conv(layers[-1], out_channels, stride=stride)
                        normalized = batchnorm(convolved)
                        rectified = lrelu(normalized, 0.2)
                        layers.append(rectified)
...
def pix2pix_func(mode, A, B, output_folder, num_epochs, direction,
                 num_gen_feats, num_dis_feats,
                 gen_layers_specs1,
                 gen_layers_specs2,
                 dis_layers_specs,
                 bdebug = 0):
    ...
    a.ngf = num_gen_feats
    a.ndf = num_dis_feats
    a.gen_layers_specs1 = gen_layers_specs1
    a.gen_layers_specs2 = gen_layers_specs2
    a.dis_layers_specs = dis_layers_specs
    if a.seed is None:
